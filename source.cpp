...
int main()
{
	//std::cout << surviveTime.asMicroseconds();
	sf::Font font;
	...
	while (window.isOpen())
	{
		...
		std::stringstream ss;
		if(highScore < lastSecond){
            highScore = lastSecond;
		}
		ss << "score: " << lastSecond << std::endl << "highscore: " << highScore;
		if(gameOver){
            ss << std::endl << "click to restart";
		}
